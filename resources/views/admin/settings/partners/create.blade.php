...
@section('content')
    <x-breadcrum name="Partners" parent="1" parent-name="Settings" page-name="New Partner" />
        ...
        <div class="container d-flex flex-column flex-lg-row">
            <div class="card card-docs flex-row-fluid mb-2 bg-gray-100">
                <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15" style="background-color:#F5F6FA;">
                    <div class="pb-10">
                        <form class="form w-100" action="{{ route('admin.settings.partners.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf

                            <div class="row mb-10">
                                <div class="col-12">
                                    <x-image-preloaded name="partner_image" />
                                </div>
                            </div>
                            @error('partner_image')
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-6 col-xl-6 col-lg-6 mb-5">
                                    ...
                                    <x-input class="{{ $errors -> has('name') ? 'is-invalid' : '' }}"
                                             id="name" name="name" value="{{ old('name') }}" placeholder="Partner Name" autofocus required />
                                    @error('name')
                                        <x-error>{{ $message }}</x-error>
                                    @enderror
                                </div>
                                <div class="col-sm-12 col-md-6 col-xl-6 col-lg-6 mb-5">
                                    ...
                                    <x-input class="{{ $errors -> has('email') ? 'is-invalid' : '' }}"
                                             id="email" type="email" name="email" value="{{ old('email') }}"  placeholder="Partner Email" />
                                    @error('email')
                                        <x-error>{{ $message }}</x-error>
                                    @enderror
                                </div>
                            ...
                            <div class="row">
                                <div class="col-sm-6 col-md-6 col-xl-6 col-lg-6 mb-10">
                                    ...
                                    <x-input class="{{ $errors -> has('cell') ? 'is-invalid' : '' }}" id="cell"
                                             name="cell" value="{{ old('cell') }}" placeholder="Partner Contact" />
                                    @error('cell')
                                        <x-error>{{ $message }}</x-error>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-10">
                                <div class="col-12">
                                    ...
                                    <x-text-area rows="4" name="address" id="address">{{ old('address') }}</x-text-area>
                                    @error('address')
                                        <x-error>{{ $message }}</x-error>
                                    @enderror
                                </div>
                            </div>
                            <x-button class="btn-primary">create Partner</x-button>
                        </form>
                    </div>
                </div>
            </div>
