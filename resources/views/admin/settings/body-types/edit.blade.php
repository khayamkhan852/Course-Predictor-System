...
@section('content')
    <x-breadcrum name="Body Types" parent="1" parent-name="Settings" page-name="edit Body Type" />
        ...
        <div class="container d-flex flex-column flex-lg-row">
            <div class="card card-docs flex-row-fluid mb-2 bg-gray-100">
                <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15" style="background-color:#F5F6FA;">
                    <div class="pb-10">
                        <form class="form w-100" action="{{ route('admin.settings.vehicle-body-types.update', $vehicleBodyType) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-6 col-xl-6 col-lg-6 mb-5">
                                    ...
                                    <x-input class="{{ $errors -> has('fuel_type') ? 'is-invalid' : '' }}"
                                             id="body_type" name="body_type" value="{{ old('body_type') ?: $vehicleBodyType->body_type }}" placeholder="Body Type" autofocus required />
                                    @error('body_type')
                                ...
                                <div class="col-sm-12 col-md-6 col-xl-6 col-lg-6 mb-5">
                                    ...
                                    <x-select-two name="branch_id" id="branch_id" required>
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{ $branch->id }}" {{ ( (old('branch_id') ?? $vehicleBodyType->branch_id) == $branch->id) ? 'selected' : '' }}>{{ $branch->name }}</option>
                                        @endforeach
                                    </x-select-two>
                                    @error('branch_id')
                                    <x-error>{{ $message }}</x-error>
                                ...
                                <div class="col-sm-12 col-md-6 col-xl-6 col-lg-6 mb-5 mt-8">
                                    <x-button class="btn-primary ms-4">Update {{ $vehicleBodyType->body_type }}</x-button>
                                </div>
                            </div>
                        </form>
                    </div>
